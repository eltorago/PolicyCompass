...
from policycompass import corpus, documents, service, store, reports, rules
from policycompass.contracts import PolicyError, fingerprint, canonical_payload, validate_run
...
class PolicyTests(unittest.TestCase):
    ...
    def test_failed_atomic_save_does_not_destroy_original(self):
        with temporary() as directory:
            ...
            with patch('policycompass.reports.os.replace',side_effect=OSError('disk full')):
    ...
    def test_signed_package_trust_and_tamper_checks(self):
        ...
        from policycompass import packages
        from policycompass.contracts import canonical
        with temporary() as directory:
    ...
    def test_cli_json_is_one_value_with_documented_exit_code(self):
        result=subprocess.run([sys.executable,'-m','policycompass','analyse',str(ROOT/'examples/policy-review/partial.md'),'--scope','Synthetic CLI','--document-status','approved','--format','json'],cwd=ROOT,capture_output=True,text=True,encoding='utf-8')
