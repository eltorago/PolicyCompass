...
from policycompass import alignment, corpus, documents, reports, service, store
from policycompass.cli import parser, execute
from policycompass.contracts import PolicyError, canonical_payload, fingerprint, validate_run
...
class AlignmentTests(unittest.TestCase):
    def test_selective_loader_preserves_controls_and_published_links(self):
        ...
        from framework_core.registry import FRAMEWORKS_BY_KEY
        ...
        full, _ = build(False)
        with patch('framework_core.loaders.extended.scf',side_effect=AssertionError('unselected source loaded')):
    ...
    def test_compare_cli_and_saved_commands_return_alignment(self):
        with temporary() as directory, patch.object(corpus, 'load', return_value=deepcopy(BASELINE)):
            ...
            with patch('policycompass.reports.render', wraps=reports.render) as render:
