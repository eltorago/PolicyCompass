...
from policycompass import ENGINE_VERSION
...
def main():
    ...
    for relative in ('dist/policycompass','work','policycompass.spec'):
        if output not in (output/relative).resolve().parents:
            raise ValueError('Build target escapes the selected output directory.')
    subprocess.run([sys.executable, '-m', 'PyInstaller', '--noconfirm', '--onedir', '--noupx', '--name', 'policycompass',
                    '--paths', str(ROOT), '--distpath', str(output / 'dist'), '--workpath', str(output / 'work'),
                    '--specpath', str(output), '--add-data', str(ROOT / 'policycompass/assets') + ';policycompass/assets',
                    ...
                    str(ROOT / 'policycompass/entry.py')], check=True, cwd=ROOT)
    payload = output / 'dist/policycompass'
    ...
    manifest = dict(status='Unsigned engineering pilot — not an approved deployment package',
                    ...
                    sourceCorpus='Not bundled. Use Framework updates or set POLICYCOMPASS_LIBRARY to an authorised local source collection.',
    ...
    (output / 'sbom.cdx.json').write_text(json.dumps(dict(bomFormat='CycloneDX', specVersion='1.5', version=1,
        metadata={'component': {'type':'application', 'name':'PolicyCompass', 'version':ENGINE_VERSION}}, components=components), indent=2), encoding='utf-8')
    (output / 'SHA256SUMS.txt').write_text('\n'.join(f['sha256'] + '  ' + f['path'] for f in inventory), encoding='utf-8')
    print(str(payload / 'policycompass.exe'))
