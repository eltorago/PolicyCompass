...
def cache_root():
    default = Path(os.environ.get('LOCALAPPDATA', str(Path.home()/'.local/share'))) / 'PolicyCompass/frameworks'
    return local_file(setting('FRAMEWORK_CACHE', str(default)))
...
def download(url, framework):
    ...
    request = Request(url, headers={'User-Agent':'PolicyCompass-framework-update/1.0','Accept':'*/*'})
