...
def training_rules():
    return json.loads((ASSETS / 'pilot-rules.json').read_text(encoding='utf-8'))['rules']


def _wa_requirements(edition=None):
    path = Path(setting("LIBRARY", str(ROOT))) / "data/corpus/wa-csp.json"
    if not path.is_file():
        raise PolicyError("The locally permitted WA CSP extract is missing. Use Framework updates, or set POLICYCOMPASS_LIBRARY to a prepared local library.", 5)
    raw = path.read_bytes()
    ...
    if edition and edition != title:
        raise PolicyError("Requested framework edition is not installed. Use python -m policycompass frameworks.", 5)
    import hashlib
    pilot = json.loads((ASSETS / 'pilot-rules.json').read_text(encoding='utf-8'))
...
def _library(keys=MANUAL_FRAMEWORKS):
    ...
    library_root = Path(setting('LIBRARY', str(ROOT)))
...
def load(framework="wa-csp", edition=None, corpus_version=None, also=()):
    ...
    if not set(selected) <= {'wa-csp', *MANUAL_FRAMEWORKS}:
        raise PolicyError('Unsupported assessment framework. Use python -m policycompass frameworks to see the available choices.', 5)
