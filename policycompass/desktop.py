...
class Desktop(ttk.Frame):
    def __init__(self, master, assessment=None):
        ...
        master.title("PolicyCompass — Framework alignment")
    ...
    def guarded(self, action):
        try:
            ...
            messagebox.showerror("PolicyCompass", str(error), parent=self)
        except Exception:
            messagebox.showerror("PolicyCompass", "The operation could not complete. Saved work is unchanged; check workspace permissions.", parent=self)
    ...
    def open_original(self):
        ...
        self.guarded(lambda: os.startfile(str(store.original_source(doc))))
